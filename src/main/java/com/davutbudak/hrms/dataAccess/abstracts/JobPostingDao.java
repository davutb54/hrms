...
public interface JobPostingDao extends JpaRepository<JobPosting, Integer> {
    @Query("select new com.davutbudak.hrms.entities.dtos.JobPostingForListDto(jp.id,e.companyName,j.name,jp.numberOfOpenPositions,jp.releaseDate,jp.deadline) from JobPosting as jp join Employer as e on e.id = jp.employerId join JobPosition as j on jp.jobPositionId = j.id where jp.active = :isActive")
    List<JobPostingForListDto> getByActive(@Param("isActive") boolean isActive);

    @Query("select new com.davutbudak.hrms.entities.dtos.JobPostingForListDto(jp.id,e.companyName,j.name,jp.numberOfOpenPositions,jp.releaseDate,jp.deadline) from JobPosting as jp join Employer as e on e.id = jp.employerId join JobPosition as j on jp.jobPositionId = j.id where jp.active = :isActive order by jp.releaseDate")
    List<JobPostingForListDto> getByActiveSortedByReleaseDate(@Param("isActive") boolean isActive);

    @Query("select new com.davutbudak.hrms.entities.dtos.JobPostingForListDto(jp.id,e.companyName,j.name,jp.numberOfOpenPositions,jp.releaseDate,jp.deadline) from JobPosting as jp join Employer as e on e.id = jp.employerId join JobPosition as j on jp.jobPositionId = j.id where jp.active = :isActive and e.id = :employerId")
